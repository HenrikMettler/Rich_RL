...
from network import GAMMA, Network, update_weights_with_policy_gradient
...
offline_net = Network(n_inputs=n_inputs, n_hidden=n_hidden, n_outputs=n_outputs,
...
for episode in range(n_epsisodes):
    ...
    for steps in range(n_steps_max):
        ...
        if env_cont_flag:
            ...
            action, log_prob = offline_net.get_action(state, rng)
        ...
        if done:
            update_weights_with_policy_gradient(network=offline_net, rewards=rewards, log_probs=log_probs)
